...
TD3_LOG_DIR = (Path("runs") / "td3_fish_baseline").as_posix()
TD3_MODEL_NAME = "td3_fish_baseline"
TD3_BUFFER_SIZE = 20_000
TD3_LEARNING_STARTS = 5_000
TD3_TRAIN_FREQ_UNIT = "episode"
TD3_GRADIENT_STEPS = 1
TD3_TAU = 0.005
TD3_POLICY_DELAY = 2
TD3_TARGET_POLICY_NOISE = 0.2
TD3_TARGET_NOISE_CLIP = 0.5
TD3_ACTION_NOISE_STD = 0.10
TD3_SAVE_REPLAY_BUFFER = False
...
def build_default_args() -> list[str]:
    ...
    args = [
        ...
        str(TD3_BUFFER_SIZE),
        "--td3-learning-starts",
        str(TD3_LEARNING_STARTS),
        "--td3-train-freq-unit",
        TD3_TRAIN_FREQ_UNIT,
        "--td3-gradient-steps",
        str(TD3_GRADIENT_STEPS),
        "--td3-tau",
        str(TD3_TAU),
        "--td3-policy-delay",
        str(TD3_POLICY_DELAY),
        "--td3-target-policy-noise",
        str(TD3_TARGET_POLICY_NOISE),
        "--td3-target-noise-clip",
        str(TD3_TARGET_NOISE_CLIP),
        "--td3-action-noise-std",
        str(TD3_ACTION_NOISE_STD),
        "--bc-weights",
        BC_WEIGHTS,
        "--log-dir",
        TD3_LOG_DIR,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train TD3 for fish path following with local obstacle avoidance."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="TD3 collects this many episodes before each episode-triggered update cycle.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--td3-buffer-size", type=int, default=None, help="Override TD3 replay buffer size.")
    parser.add_argument(
        "--td3-learning-starts",
        type=int,
        default=None,
        help="Override TD3 warmup steps before gradient updates start.",
    )
    parser.add_argument(
        "--td3-train-freq-steps",
        type=int,
        default=None,
        help="Override TD3 training trigger count. The unit is controlled by --td3-train-freq-unit.",
    )
    parser.add_argument(
        "--td3-train-freq-unit",
        type=str,
        ...
        default=None,
        help="Unit for TD3 training triggers: environment steps or completed episodes.",
    )
    parser.add_argument(
        "--td3-gradient-steps",
        type=int,
        default=None,
        help="Override TD3 gradient steps per training trigger.",
    )
    parser.add_argument(
        "--td3-tau",
        type=float,
        default=None,
        help="Override TD3 target network smoothing coefficient.",
    )
    parser.add_argument(
        "--td3-policy-delay",
        type=int,
        default=None,
        help="Override TD3 delayed policy update interval.",
    )
    parser.add_argument(
        "--td3-target-policy-noise",
        type=float,
        default=None,
        help="Override TD3 target policy smoothing noise std.",
    )
    parser.add_argument(
        "--td3-target-noise-clip",
        type=float,
        default=None,
        help="Override TD3 target policy smoothing noise clip.",
    )
    parser.add_argument(
        "--td3-action-noise-std",
        type=float,
        default=None,
        help="Exploration Gaussian noise std applied to TD3 actions during data collection.",
    )
    parser.add_argument(
        "--use-lora",
        action=argparse.BooleanOptionalAction,
...
def _make_td3_action_noise(env, noise_std: float) -> NormalActionNoise:
    action_space = env.action_space
    action_shape = getattr(action_space, "shape", None)
    if not action_shape:
        raise ValueError("TD3 requires a continuous Box action space with a known shape.")
...
def load_bc_actor_state_dict_into_td3_policy(
    policy,
...
def main() -> None:
    ...
    td3_buffer_size = TD3_BUFFER_SIZE if args.td3_buffer_size is None else int(args.td3_buffer_size)
    td3_learning_starts = TD3_LEARNING_STARTS if args.td3_learning_starts is None else int(args.td3_learning_starts)
    td3_train_freq_steps = 1 if args.td3_train_freq_steps is None else int(args.td3_train_freq_steps)
    td3_train_freq_unit = TD3_TRAIN_FREQ_UNIT if args.td3_train_freq_unit is None else str(args.td3_train_freq_unit)
    td3_gradient_steps = TD3_GRADIENT_STEPS if args.td3_gradient_steps is None else int(args.td3_gradient_steps)
    td3_tau = TD3_TAU if args.td3_tau is None else float(args.td3_tau)
    td3_policy_delay = TD3_POLICY_DELAY if args.td3_policy_delay is None else int(args.td3_policy_delay)
    td3_target_policy_noise = (
        TD3_TARGET_POLICY_NOISE if args.td3_target_policy_noise is None else float(args.td3_target_policy_noise)
    )
    td3_target_noise_clip = (
        TD3_TARGET_NOISE_CLIP if args.td3_target_noise_clip is None else float(args.td3_target_noise_clip)
    )
    td3_action_noise_std = (
        TD3_ACTION_NOISE_STD if args.td3_action_noise_std is None else float(args.td3_action_noise_std)
    )
    ...
    if td3_buffer_size <= 0:
        raise ValueError("--td3-buffer-size must be positive.")
    if td3_learning_starts < 0:
        raise ValueError("--td3-learning-starts must be non-negative.")
    if td3_train_freq_steps <= 0:
        raise ValueError("--td3-train-freq-steps must be positive.")
    if td3_train_freq_unit not in {"step", "episode"}:
        raise ValueError("--td3-train-freq-unit must be either 'step' or 'episode'.")
    if td3_gradient_steps == 0:
        raise ValueError("--td3-gradient-steps cannot be 0.")
    if td3_tau <= 0.0 or td3_tau > 1.0:
        raise ValueError("--td3-tau must be within (0, 1].")
    if td3_policy_delay <= 0:
        raise ValueError("--td3-policy-delay must be positive.")
    if td3_target_policy_noise < 0.0:
        raise ValueError("--td3-target-policy-noise must be non-negative.")
    if td3_target_noise_clip < 0.0:
        raise ValueError("--td3-target-noise-clip must be non-negative.")
    if td3_action_noise_std < 0.0:
        raise ValueError("--td3-action-noise-std must be non-negative.")
    ...
    if episode_cycle_enabled and td3_train_freq_unit == "episode" and args.td3_train_freq_steps is None:
        td3_train_freq_steps = int(rollout_episodes_per_update)
    ...
    config_payload["algorithm"] = "td3"
    config_payload["td3"] = {
        "buffer_size": int(td3_buffer_size),
        "learning_starts": int(td3_learning_starts),
        "train_freq_steps": int(td3_train_freq_steps),
        "train_freq_unit": str(td3_train_freq_unit),
        "gradient_steps": int(td3_gradient_steps),
        "tau": float(td3_tau),
        "policy_delay": int(td3_policy_delay),
        "target_policy_noise": float(td3_target_policy_noise),
        "target_noise_clip": float(td3_target_noise_clip),
        "action_noise_std": float(td3_action_noise_std),
        "save_replay_buffer": bool(TD3_SAVE_REPLAY_BUFFER),
    }
    ...
    print(
        "TD3 train frequency: "
        f"{int(td3_train_freq_steps)} {str(td3_train_freq_unit)}(s) per update trigger"
    )
    print(
        "TD3 exploration noise: "
        f"std={td3_action_noise_std:.3f}, policy_delay={td3_policy_delay}, "
        f"target_noise={td3_target_policy_noise:.3f}, noise_clip={td3_target_noise_clip:.3f}"
    )
    if args.use_lora:
        print(
    ...
    action_noise = _make_td3_action_noise(env, td3_action_noise_std)
    ...
    if direct_load_resume_allowed:
        ...
        if replay_buffer_path is not None:
            ...
            print("Replay buffer sidecar not found. TD3 resume will continue with a fresh replay buffer.")
    else:
        model = TD3(
            policy=policy_spec,
            env=env,
            ...
            train_freq=(int(td3_train_freq_steps), str(td3_train_freq_unit)),
            gradient_steps=int(td3_gradient_steps),
        ...
        if resume_path is not None or resume_policy_weights_path is not None:
            ...
            loaded_keys, skipped_keys = load_bc_actor_state_dict_into_td3_policy(model.policy, actor_state_dict)
            if not loaded_keys:
                raise RuntimeError(f"No actor parameters were loaded from BC checkpoint: {bc_weights_path}")
            print(f"Loaded {len(loaded_keys)} BC actor parameter tensors into TD3.")
            if skipped_keys:
                print(f"Skipped {len(set(skipped_keys))} BC tensors incompatible with the TD3 actor head.")
