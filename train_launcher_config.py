...
SCENARIO_CYCLE_DIR = (Path("scenarios") / "large_pool_dataset_200" / "train" / "json").as_posix()
BC_WEIGHTS = (
    Path("runs")
    / "bc_pretrain"
    / "large_pool_bc_v2"
...
# These launcher knobs are shared by the SAC / PPO / TD3 training entrypoints.
...
SAC_LOG_DIR = (Path("runs") / "sac_lora_fish_baseline").as_posix()
SAC_MODEL_NAME = "sac_lora_fish_baseline"
SAC_TRAIN_FREQ_UNIT = "episode"

PPO_LOG_DIR = (Path("runs") / "ppo_lora_fish_baseline").as_posix()
PPO_MODEL_NAME = "ppo_lora_fish_baseline"


def build_wrapper_default_args(
    *,
    algo: str,
    log_dir: str,
    model_name: str,
    ...
    if algo_name not in {"ppo", "sac"}:
        raise ValueError(f"Unsupported launcher algorithm: {algo!r}")
