...
SCENARIO_CYCLE_DIR = _first_existing_path(
    Path("ppo_train_40") / "scenarios" / "train" / "json",
    Path("scenarios") / "large_pool_dataset_200" / "train" / "json",
).as_posix()
PPO_BC_WEIGHTS = _first_existing_path(
    Path("ppo_train_40") / "weights" / "bc_large_pool_1_20__60_80_epoch020_actor.pth",
    Path("runs") / "bc_pretrain" / "bc20_40_60_80_100_ppo" / "bc_large_pool_1_20__60_80_epoch020_actor.pth",
).as_posix()
...
PPO_LOG_DIR = (Path("runs") / "ppo_train_40").as_posix()
PPO_MODEL_NAME = "ppo_train_40"
